...
protocol 可乘載的: CustomStringConvertible {
    var 最大乘客數: Int { get }
    var 牌號: String { get }
    var 時速: Double { get }
...
extension 可乘載的 {
    var description: String {
        return "\(Self.deviceName)牌號「\(self.牌號)」"
    }
    
    public func 計算時間(目的地距離: Double) -> Int {
        let time = 目的地距離 / 時速
        let min = time * 60
...
struct 汽車: 可乘載的 {
    ...
    static var deviceName: String = "🚘 汽車"
}
...
struct 機車: 可乘載的 {
    ...
    static var deviceName: String = "🛵 機車"
}
...
struct 直升機: 可乘載的 {
    ...
    static var deviceName: String = "🚁 直升機"
}
...
struct 載客管理員 {
    ...
    mutating func 派車(人數: Int, 客人距離: Double) -> 可乘載的? {
        guard let deviceIndex = (self.可用載客車.firstIndex { (item) -> Bool in
            if item.最大乘客數 >= 人數 {
                return true
            }
            return false
        ...
        let device = self.可用載客車.remove(at: deviceIndex)
        print("\(device)正在前往您的地點，預計 \(device.計算時間(目的地距離: 客人距離)) 分鐘後抵達。")
        return device
    }
}
...
public func runHomeworkB(){
    ...
    let _ = carManager.派車(人數: 1, 客人距離: 10.9)
    let _ = carManager.派車(人數: 5, 客人距離: 80.2)
    let _ = carManager.派車(人數: 7, 客人距離: 5.3)
    let _ = carManager.派車(人數: 2, 客人距離: 9.2)
    let _ = carManager.派車(人數: 6, 客人距離: 2.1)
    let _ = carManager.派車(人數: 2, 客人距離: 0.7)
    let _ = carManager.派車(人數: 1, 客人距離: 18.5)
    let _ = carManager.派車(人數: 4, 客人距離: 38.6)
    let _ = carManager.派車(人數: 4, 客人距離: 7.3)
    let _ = carManager.派車(人數: 1, 客人距離: 222)
