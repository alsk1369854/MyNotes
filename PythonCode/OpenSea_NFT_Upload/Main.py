    ...
    for image in sorted_image_list:
        ...
        while True:
            try:
                ...
                pass
            sleep(5)
        # waiting_element_displayed(driver, '//p [text()="SHARE"]')
        # 紀錄
        print(image)
        Path(IMAGES_DIR_PATH + '/' + image).rename(IMAGES_DIR_PATH + '/uploaded/' + image)
...
def upload_item(image):
    ...
    # image
    # sleep(5)
    inputs[1].send_keys(IMAGES_DIR_PATH + '/' + image)
    # image name
    # sleep(5)
    ...
    # image descript
    # sleep(5)
    # waiting_element_displayed(driver, '//textarea')[0].send_keys(IMAGES_DESCRIPTION)
    # create item
    sleep(random.randint(5,5))
    # waiting_element_displayed(driver, '//button [text()="Create"]')[0].click()
